...
int main(int argc, const char** argv) {
	...
	dst = src;
	  ...
	  //1b. COMPUTE AND DISPLAY HOUGH SPACE
	...
	cout << "Viola Jones method detected " << vjones.size() << " potential dartboards." << endl;
	//carry out hough circle detection
	houghCircle(src, circles);
	cout << "Hough Transform (circles) detected " << circles.size() << " potential dartboards." << endl;
	//call function to generate matrix of line intersections
	houghLineIntersects(src, intersectCoords);

    //1d. 'VOTE' FOR INDIVIDUAL VIOLA JONES BOXES IF CIRCLES OR LINE INTERSECTS ARE DISCOVERED IN THE SAME PLACE
	...
	float xthresh = 0.25;
	float ythresh = 0.25;
	for(int c=0; c<circles.size(); c++) {// c denotes the current circle being looked at
		for(int r=c; r<vjones.size(); r++) {// r denotes the current rectangle being looked at
			// check the x coord of circle centre point lies within vjones box
			if(circles[c][0] > (vjones[r].x + xthresh*vjones[r].width) && circles[c][0] < (vjones[r].x +
          vjones[r].width*(1 - xthresh))) {
	...
	cout << "Viola Jones & Hough circles agree at " << circle_overlaps << " locations." << endl;
	// repeat above code to add vjones points for high line intersection areas
	int intersect_overlaps = 0;
	for(int i=0; i<intersectCoords.size(); i++) {  // i denotes the current line intersection point being looked at
		for(int r=i; r<vjones.size(); r++) { // r denotes the current rectangle being looked at
			// check the x coord of line intersect point lies within vjones box
			if(intersectCoords[i].x > (vjones[r].x + xthresh*vjones[r].width) && intersectCoords[i].x < (vjones[r].x +
          vjones[r].width*(1 - xthresh))) {
				// check the y coord of line intersect point lies within vjones box
				if(intersectCoords[i].y > (vjones[r].y + ythresh*vjones[r].height) && intersectCoords[i].y < (vjones[r].y +
            vjones[r].height*(1 - ythresh))) {
	...
	cout << "Hough Transform (Lines) detected " << intersectCoords.size() << " areas with high numbers of line "
      "intersections." << endl;
	cout << "Viola Jones & Hough lines agree at " << intersect_overlaps << " locations." << endl;
	...
	for(int r=0; r<winners.size(); r++) {
		rectangle(dst, Point(winners[r].x, winners[r].y), Point(winners[r].x + winners[r].width, winners[r].y +
        winners[r].height), Scalar( 0, 255, 0 ), 2);
	}
	imshow("Detected Dart Boards", dst);
	waitKey(0);

    // 1e. DEFINE GROUNDS & COMPUTE F1 SCORE
	//ask for numboards
	...
	for (int i=0; i < numboards; i++) {
		for (int j=0; j < winners.size(); j++) {
			bool ground_intersects = ((grounds[i] & winners[j]).area() > union_area*winners[j].area());
...
void sobel(Mat &input, Mat &sobelOutput) {
	...
	for ( int i = 0; i < input.rows; i++ ) {
		for( int j = 0; j < input.cols; j++ ) {
			...
			sobelOutput.at<double>(i, j) =  std::sqrt((sum_x*sum_x) + (sum_y*sum_y));
			if (sobelOutput.at<double>(i, j) > sobel_threshold) {
				sobelOutput.at<double>(i, j) = 200;
			}			
			else sobelOutput.at<double>(i, j) = 0;
			sobelGradient.at<double>(i,j) = (atan2 (sobelOutput_y.at<uchar>(i, j),
                                              sobelOutput_x.at<uchar>(i, j)) * 180 / 3.14159265);
		}
	}
}
...
void showHough(Mat &img_ori, Mat &normal_accum) {
  // 2a. DECLARE VARIABLES
	...
	double centre_x = w/2;
	double centre_y = h/2;
   // 2b. SETUP ACCUMULATOR MATRIX
	//establish max r value (max distance from centre to corner of image i.e. pythag)
   ...
   // 2c. COMPUTE r FOR EVERY ANGLE AT EVERY DETECTED EDGE & POPULATE ACCUMULATOR MATRIX
	//loop through y
	for(int y=0; y<h; y++) {
		//loop through x
		for(int x=0; x<w; x++) {
			...
			if(img_edge.at<uchar>(y, x) > 0) {
				...
				for(int t=0; t<180; t++) {
					...
					double r = ( ((double)x - centre_x) * cos((double)t * DEG2RAD)) + (((double)y - centre_y) * sin((double)t * DEG2RAD));
   ...
   // 2d. OUTPUT HOUGH SPACE IMAGE (VISUAL REPRESENTATION OF ACCUMULATOR)
...
void violajones(Mat &src, vector<Rect> &boards) {
	String cascade_name = "Merge2.xml";
	CascadeClassifier cascade;
	// 3a. Load the Strong Classifier in a structure called `Cascade'
	// Classifier e.g. a set of parameters that define a certain feature to be searched for.
	cascade.load( cascade_name );
	Mat src_gray;
	// 3b. Prepare Image by turning it into Grayscale and normalising lighting
	cvtColor( src, src_gray, CV_BGR2GRAY );
	equalizeHist( src_gray, src_gray );
	// 3c. Perform Viola-Jones Object Detection
	cascade.detectMultiScale( src_gray, boards, 1.1, 1, 0|CV_HAAR_SCALE_IMAGE, Size(50, 50), Size(500,500) );
}
...
void houghCircle(Mat &src, vector<Vec3f> &circles) {
	...
	HoughCircles( src_gray, circles, CV_HOUGH_GRADIENT, 1, src_gray.rows/4, uThreshold, lThreshold, 50, 600 );
}
...
void houghLineIntersects(Mat &src, vector<Point2f> &intersectCoords) {
   ...
   // 4b. COMPUTE INTERSECT POINTS FOR DETECTED LINES
	...
	for(int i=0; i < lines.size(); i++) {
		for(int j=i; j<lines.size(); j++) {
			intersection(Point(lines[i][0], lines[i][1]), Point(lines[i][2], lines[i][3]), Point(lines[j][0], lines[j][1]),
                   Point(lines[j][2], lines[j][3]), intersect);
			// exclude intersects that happen outside the borders of the image
			if(intersect.x > 0 && intersect.x < w && intersect.y > 0 && intersect.y < h)
				// write intersect coords to intersects vector
				intersects.push_back(Point2f(intersect));
		}
	}
   // 4c. Determine size of intersect_count matrix for an increment of (pixelStep) pixels
	// Set incremental step in terms of pixels for cell divisions
	int pixelStep = 10;
	// divides source image into sections by pixelstep
	Mat intersectCount = (Mat_<double>(round((src.rows)/pixelStep),round((src.cols)/pixelStep)));
   ...
   // 4d. LOOP THROUGH EACH CELL OF NEWLY DIVIDED IMAGE MATRIX AND COUNT POINTS OF INTERSECTION
	...
	for(int i=0; i<(intersectCount.rows); i++) {
		...
		for(int j=0; j<(intersectCount.cols); j++) {
			...
			for(int v=0; v<intersects.size(); v++) {
				//check image for detected edge (ie pixel above threshold)
				if ((intersects[v].x > j*pixelStep ) && (intersects[v].x < j*pixelStep+pixelStep)
            && (intersects[v].y > i*pixelStep ) && (intersects[v].y < i*pixelStep+pixelStep)) {
	...
	for (int r=0; r<intersectCount.rows; r++) {
    for (int c=0; c<intersectCount.cols; c++) {
      if (intersectCount.at<double>(r, c) > (intersect_thresh * max_intersects)) {
...
void CallBackFunc(int event, int x, int y, int flags, void* userdata) {
  ...
  if  ( event == EVENT_LBUTTONDOWN ) {
    if (boxStarted == false) {
      ...
      boxStarted = false;
      rectangle(dst, Point(grounds[a].x, grounds[a].y), Point(grounds[a].x + grounds[a].width, grounds[a].y + grounds[a].height), Scalar( 255, 0, 0 ), 2);
      imshow("Detected Dart Boards", dst);

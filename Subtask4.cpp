...
int main(int argc, const char **argv) {
  ...
  for (int i = 0; i <
    ...
    for (int r = i; r <
      ...
      if (intersectCoords[i].x > (vjones[r].x + xthresh * vjones[r].width) &&
          intersectCoords[i].x < (vjones[r].x +
                                  vjones[r].width *
                                  (1 - xthresh))) {
        ...
        if (intersectCoords[i].y > (vjones[r].y + ythresh * vjones[r].height) &&
            intersectCoords[i].y < (vjones[r].y +
                                    vjones[r].height *
                                    (1 -
                                     ythresh))) {
  ...
  cout << "Hough Transform (Lines) detected " << intersectCoords.size()
       << " areas with high numbers of line "
           "intersections." << endl;
  ...
  for (int r = 0; r < winners.size(); r++) {
    rectangle(dst, Point(winners[r].x, winners[r].y),
              Point(winners[r].x + winners[r].width, winners[r].y +
                                                     winners[r].height),
              Scalar(0, 255, 0), 2);
  }
